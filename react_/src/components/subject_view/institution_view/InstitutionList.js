...
export default class InstitutionList extends Component {
  render() {
    const { name1, address, city, state, country, id } = this.props.institution;
    return (
      <div className="pattern">
        <label>
          <input
            type="checkbox"
            onChange={this.props.changeInstitution.bind(this, id, name1)}
          />{" "}
          Click Here to select.
          {console.log(id)}
        </label>
        <center>
          <h3 style={{ color: "#303F9F" }}>{name1}</h3>
          {address}
        </center>
